...
export const updateFeedbackTypeById = createAsyncThunk<void,
  {
    id?: string;
    name?: string;
    description?: string;
  ...
  }>('feedback-types/update-by-id', async (payload, thunkAPI) => {
  thunkAPI.dispatch(toggleSpinnerOn());
  try {
    const response = await axios.put(`api/feedback-types/${payload.id}`, {
      name: payload.name,
